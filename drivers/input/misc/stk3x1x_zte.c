...
#ifdef STK_TUNE0
#if 1 // wanghaifei 20130801, for calibration during call
	ps_data->psa = 0;
	ps_data->psi = 0xffff;	
	ps_data->psi_set = 0;
#endif
                        ...
                        const struct i2c_device_id *id)
{
	...
	ret = stk3x1x_i2c_read_data(ps_data->client, STK_PDT_ID_REG, 1, &value);
	if((ret < 0 ) || (value != STK3210_STK3310_PID))
	{
		printk(KERN_ERR "%s fail to read sensortek sensor PDT_ID , ret=0x%x value=0x%x\n", __func__, ret, value);
		goto err_match_id;
	}
	printk(KERN_INFO "%s read sensortek sensor PDT_ID 0x%x\n", __func__, value);
#ifdef STK_POLL_PS			
