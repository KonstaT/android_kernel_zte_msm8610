...
struct ppp_status {
	//int busy;  //QCOMM PATCH 2014-4-23, resolve MDP hung
	bool wait_for_pop;
	//spinlock_t ppp_lock;  //QCOMM PATCH 2014-4-23, resolve MDP hung
...
{
	int ret = 1;
	//int wait;   //QCOMM PATCH 2014-4-23, resolve MDP hung
	//unsigned long flag;  //QCOMM PATCH 2014-4-23, resolve MDP hung

	/*
	 * wait 40 ms for ppp operation to complete before declaring
	 * the MDP hung
	 */
	//QCOMM PATCH 2014-4-23, resolve MDP hung
...
{
	//spin_lock(&ppp_stat->ppp_lock);   //QCOMM PATCH 2014-4-23, resolve MDP hung
	//ppp_stat->busy = false;           //QCOMM PATCH 2014-4-23, resolve MDP hung
	//spin_unlock(&ppp_stat->ppp_lock); //QCOMM PATCH 2014-4-23, resolve MDP hung
	complete(&ppp_stat->ppp_comp);
	//mdp3_irq_disable_nosync(type);    //QCOMM PATCH 2014-4-23, resolve MDP hung
}
...
{
	//unsigned long flag;              //QCOMM PATCH 2014-4-23, resolve MDP hung
	//mdp3_irq_enable(MDP3_PPP_DONE);  //QCOMM PATCH 2014-4-23, resolve MDP hung
	...
	ppp_enable();
	mdp3_ppp_pipe_wait();
	mdp3_irq_disable(MDP3_PPP_DONE);  //QCOMM PATCH 2014-4-23, resolve MDP hung
}
...
void mdp3_start_ppp(struct ppp_blit_op *blit_op)
{
	...
	//do { } while (mdp3_ppp_pipe_wait() <= 0);  //QCOMM PATCH 2014-4-23, resolve MDP hung
	//QCOMM PATCH 2014-4-23, resolve MDP hung
...
int mdp3_ppp_res_init(struct msm_fb_data_type *mfd)
{
	...
	//spin_lock_init(&ppp_stat->ppp_lock);  //QCOMM PATCH 2014-4-23 resolve MDP hung
