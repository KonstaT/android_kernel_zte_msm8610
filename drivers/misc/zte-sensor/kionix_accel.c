...
//added by yangze 20120919 start
/* Returns the enable state of device */
static ssize_t attr_accel_get_enable(struct device *dev,
				struct device_attribute *attr, char *buf)
{
...
/* Allow users to enable/disable the device */
static ssize_t attr_accel_set_enable(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct kionix_accel_driver *acceld = dev_get_drvdata(dev);
	struct input_dev *input_dev = acceld->input_dev;
...
/* Returns currently selected poll interval (in ms) */
static ssize_t attr_accel_get_delay(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct kionix_accel_driver *acceld = dev_get_drvdata(dev);
...
/* Allow users to select a new poll interval (in ms) */
static ssize_t attr_accel_set_delay(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct kionix_accel_driver *acceld = dev_get_drvdata(dev);
	struct input_dev *input_dev = acceld->input_dev;
...
static struct device_attribute attributes[] = {
	__ATTR(pollrate_ms, 0644, attr_accel_get_delay,attr_accel_set_delay),
	__ATTR(enable_device, 0644, attr_accel_get_enable, attr_accel_set_enable),
};
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (err) {
		KMSGERR(&acceld->client->dev, "%s: create_sysfs_interfaces returned err = %d. Abort.\n", __func__, err);
		goto err_free_irq;
	}
	//added by yangze 20120919 end
#ifdef    CONFIG_HAS_EARLYSUSPEND
...
#endif /* CONFIG_HAS_EARLYSUSPEND */
	remove_sysfs_interfaces(&client->dev);// added by yangze 20120919
