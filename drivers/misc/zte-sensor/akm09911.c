...
static int akm_power_init(struct akm_compass_data *data, bool on)
{
        int rc;
	int vol_uv;
	int enabled = 0;
        ...
        if (!on) {
                ...
                if (regulator_count_voltages(data->vdd) > 0)
                        regulator_set_voltage(data->vdd, 0, AKM_VDD_MAX_UV);

                regulator_put(data->vdd);

                if (regulator_count_voltages(data->vio) > 0)
                        regulator_set_voltage(data->vio, 0, AKM_VIO_MAX_UV);
		...
		dev_info(&data->i2c->dev, "vdd current voltate is %duv, enabled = %d ", vol_uv, enabled);
		if ((vol_uv >= AKM_VDD_MIN_UV) && (vol_uv <= AKM_VDD_MAX_UV)) {
			dev_info(&data->i2c->dev,"no need to set vdd voltage again");
		} else if (regulator_count_voltages(data->vdd) > 0) {
			rc = regulator_set_voltage(data->vdd, AKM_VDD_MIN_UV,
					AKM_VDD_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
		...
		if (enabled == 1) {
			dev_info(&data->i2c->dev,"no need to enable vdd voltage again");
		...
		dev_info(&data->i2c->dev, "vio current voltate is %duv, enabled = %d ", vol_uv, enabled);
		if ((vol_uv >= AKM_VIO_MIN_UV) && (vol_uv <= AKM_VIO_MAX_UV)) {
			dev_info(&data->i2c->dev,"no need to set vio voltage again");
		} else if (regulator_count_voltages(data->vio) > 0) {
			rc = regulator_set_voltage(data->vio, AKM_VIO_MIN_UV,
					AKM_VIO_MAX_UV);
			if (rc) {
				dev_err(&data->i2c->dev,
		...
		if (enabled == 1) {
			dev_info(&data->i2c->dev,"no need to enable vio voltage again");
...
{
        if(0xff == chipid) {
                printk("not create akm09911 proc file due to unconnected\n");
                return 0;
        } 
